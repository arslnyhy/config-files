...
def create_cve_files(dry_run=False):
    destination_folder = "/home/user/cveasy/tests/cisco/2025/ios"
    ...
    if dry_run:
        ...
        for cve in cve_names:
            filename = f"{cve.lower().replace('-', '')}.yaml"
    else:
        ...
        for cve in cve_names:
            ...
            with open(file_path, 'w') as f:
                f.write(f'# Placeholder for rule tester\n')
            created_files += 1
        print(f"Created {created_files} files in {destination_folder}")

def main():
    parser = argparse.ArgumentParser(description='Create CVE test files')
